...
from utils.mat2pic import trans_separate, GeneralDataset_diff, ValDataset_diff, TestDataset_diff
...
def train(idx_train, num_data_train, net_name):
    ...
    model_path = os.path.join(project_path, 'model', 'source', 'problem3', 'source', 'resnet18', 'best_model')
    os.makedirs(os.path.dirname(model_path), exist_ok=True)

    log_dir = os.path.join(project_path, 'log', 'fpn', 'source', 'problem3', net_name, num_data_train, str(idx_train))
    os.makedirs(log_dir, exist_ok=True)
    writer = SummaryWriter(log_dir)

    dataset = GeneralDataset_diff(trans_separate, resize_shape=(200, 200))
    ...
    scheduler = WarmupMultiStepLR(optimizer, milestones=[3125, 9375, 15625], warmup_iters=len(train_loader))
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % model_valid_interval == 0:
            ...
            if mae < best_mae:
                best_mae = mae
                model_save_path = os.path.join(project_path, "model", 'source', "problem3", net_name, num_data_train, str(idx_train))
...
def test(idx_train, num_data_train, name_net):
    ...
    model_path = os.path.join(project_path, "model", 'source', "problem3",
                              name_net, num_data_train, str(idx_train), "best_model")

    target_dataset_test = TestDataset_diff(trans_separate, resize_shape=(200, 200))
    target_test_loader = DataLoader(target_dataset_test, batch_size=64, shuffle=False, drop_last=False)
    print("model path:", model_path)
